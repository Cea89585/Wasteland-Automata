...
import { Users, Check, ChevronsRight } from 'lucide-react';
...
export default function CommunityPanel() {
    ...
    const { inventory, completedQuests, playerStats, isResting, smeltingQueue } = gameState;
    ...
    const canComplete = (questId: string) => {
        ...
        for (const req of quest.requirements) {
            if (inventory[req.item] < req.amount) {
                return false;
            }
        }
        return true;
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <div className="flex flex-col gap-1 text-sm">
                                                {quest.requirements.map(req => (
                                                    <div key={req.item} className="flex items-center justify-between p-2 rounded bg-background/50">
                                                        <span className="flex items-center">
                                                            {allIcons[req.item]} {itemData[req.item].name}
                                                        </span>
                                                        <span className={inventory[req.item] >= req.amount ? 'text-green-400' : 'text-destructive'}>
                                            ...
                                            <div className="flex flex-wrap gap-2 text-sm">
                                                {quest.rewards.map((reward, index) => (
                                                    <div key={index} className="flex items-center gap-2 p-2 rounded bg-background/50">
                                                        {reward.type === 'item' ? allIcons[reward.item] : resourceIcons.silver}
                                                        <span>
